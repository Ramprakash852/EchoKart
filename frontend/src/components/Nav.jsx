...
function Nav() {
  ...
  return (
    <div className="w-[100vw] h-[70px] bg-[#ecfafaec] z-10 fixed top-0 flex items-center justify-between px-[30px] shadow-md shadow-black">
      <div className="w-[20%] lg:w flex  items-center justify-start gap-[10px]">
        <img src={logo} alt="" className="w-[30px]" />
        <h1 className="text-[25px] text-[black] font-sans "> EchoCart</h1>
      </div>
      <div className="w-[50%] lg:w-[40%] hidden md:flex">    
        <ul className="flex items-center justify-center gap-[19px] text-[white]">
          ...
          <li onClick={()=>navigate("/collection")} className="text-[15px] hover:bg-slate-500 cursor-pointer bg-[#000000c9] py-[10px] px-[20px] rounded-2xl">
            COLLECTIONS
          </li>
      ...
      {showSearch && (
        <div className="w-[100%] h-[80px] bg-[#d8f6f9dd] absolute top-[100%] left-0 right- flex items-center justify-center">
          <input
            className="lg:w-[50%] w-[80%] h-[60%] bg-[#233533] rounded-[30px] px-[50px] placeholder:text-white text-[18px]"
            type="search"
            placeholder="search"
            onChange={(e)=>{
      ...
      <div className="w-[100vw] h-[90px] flex items-center justify-between px-[20px] text-[12px] fixed bottom-0 left-0 bg-[#191818] md:hidden">
        ...
        <button className="text-[white] flex items-center justify-center flex-col gap-[2px]" onClick={()=>{navigate("/contact")}} ><IoMdContact  className="w-[28px] h-[28px] text-[white] md:hidden" /> Contact</button>
        <button className="text-[white] flex items-center justify-center flex-col gap-[2px]"><MdOutlineShoppingCart className="w-[28px] h-[28px] text-[white] md:hidden" onClick={()=>{navigate("/cart")}} />Cart</button>
